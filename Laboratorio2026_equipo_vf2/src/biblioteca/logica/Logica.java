...
public class Logica {
	...
	HeapPriorityQueue<Integer, String> colavmenor = new HeapPriorityQueue<>();
	...
	public Logica(ProbeHashMap<String, Libro> catalogo, ProbeHashMap<String, Socio> socios,
		...
		for (LinkedPositionalList<Prestamo> lista : prestamosActivos.values()) { // Cargamos las listamos de prestamos
			...
			for (Prestamo p : lista) {// exploramos prestamo por prestamo de dicha lista
				...
				if (cant == null) {// si el contador no exite osea que ese libro no fue prestado antes
	...
	public boolean prestar(String nroSocio, String isbn) {
		...
		LocalDate fechadevencimiento = fechaprestamo.plusDays(Constante.PRESTAMOS_MAX_DIAS);// Con el metodo plusDays le
																							// sumo 14 dias ala fecha
		...
		if (cant == null) {// si no existe el contador para ese libro lo agragamos al mapa y ya que esto se
							// hace luego de un prestamo lo seteamos como que fue prestado una ves
			conteo.put(isbn, 1);
		} else {// si ya existia dicho contador dentro del mapa solo lo actualizo ya que ese
				// ejemplar de libro fue nuevamente rentado
			conteo.put(isbn, cant + 1);
		}
	...
	public LinkedPositionalList<Libro> librosMasSolicitados(int n) {
		LinkedPositionalList<Libro> resultados = new LinkedPositionalList<>();// creo una nueva lista pocicional para
																				// guardar el ranking

		if (conteo.isEmpty()) {// pregunto si el mapa esta vacio si es asi significa que aun no hay ni un
								// prestamo previo hecho
			return resultados;// y si es asi retorno la lista vacia
		}

		HeapPriorityQueue<Integer, String> coladeorden = new HeapPriorityQueue<>();//creo un cola de prioridad para ordenar el ranking
		for (String llave : conteo.keySet()) {//en esto coloco la llave en si que es la cantidad de veces que fue restado ese libro
			coladeorden.insert(-conteo.get(llave), llave);//guardo ahora ciendo el valor el isbn y la llave el valor negado de la cantidad de veces que fue rentado el libro de manera negada para engañar y que lo ordena de mayor a menor
		}

		int posicion = 1;//declaro la pocicion del ranking
		while (!coladeorden.isEmpty() && posicion <= n) {//este bucle repetira hasta que halla de terminar el numero de ranking que pidio el usuario o hasy que la cola de prioridad quede vacia
			String isbn = coladeorden.removeMin().getValue();//desencolo y recuperando el valor del isbn
			Libro libro = catalogo.get(isbn);//con el isbn busco el libro
			resultados.addLast(libro);//guardo el libro en la lista del rankig quedadome en el orden de mayor a menor
			posicion++;
		}

		return resultados;
	}
//el geter me sirve para debolver el mapa de contaodre de manera completa para que la aplicaion la use
	public ProbeHashMap<String, Integer> getConteo() {
		return conteo;
	}
	...
	public HeapPriorityQueue<Integer, String> pasarcolavmenor() {
		return colavmenor;
