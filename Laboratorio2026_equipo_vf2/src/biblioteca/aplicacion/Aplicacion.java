        ...
        do {
            ...
            switch (opcion) {
                ...
                case Constante.OPCION_RANKING:
                	...
                	int n = Interfaz.pedirN();//pido al usuario que inserte la cantidad de libros a mostrar para el ranking
                    LinkedPositionalList<Libro> ranking = logica.librosMasSolicitados(n);//copio la lista pocicional qeu fue creada en la logica
                    if (ranking.isEmpty()) {//preguto si dicha lista esta vacia si es asi le aviso al usuario que no hay datos de prestamos
                        Interfaz.mostrarMensaje("no hay datos de prestamos");
                    } else {//si la lista cuenta con almenos algunos o un prestamos lo muestro usando la interfaz
                        Interfaz.mostrarMensaje("top " + n + " libros mas solicitados:");
                        Interfaz.mostrarRanking(ranking, logica.getConteo());//y eso lo hago llamando ala interfaz y a dicho metodo para mostrar el ranking pasandole la lista del rankig y el mapa de contadores
                    }
                    break;
